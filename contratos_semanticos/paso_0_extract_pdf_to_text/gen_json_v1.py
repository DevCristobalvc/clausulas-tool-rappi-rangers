...
# Alternativas disponibles según tu "llama list": embeddinggemma:latest, mxbai-embed-large:latest,
# nomic-embed-text:latest. Si cambias, ajústalo aquí o con variable de entorno.
...
def split_segments(text: str) -> List[Dict]:
    ...
    text = text.replace("\r\n", "\n")
    parts = []
    # split por posibles encabezados de cláusula
    pattern = re.compile(r"(?im)^(?:cl[áa]usula\s+\d+|t[ée]rmino|vigencia|pagos?|exclusividad)\b.*$")
...
\"\"\"{ctx}\"\"\""""
    ...
    # si el LLM dijo NO pero encontramos señales fuertes, forzamos SI
    ...
    detalle = excerpt_from_segments(segments, ["pago","bono","upfront","desembolso"], 800)
...
\"\"\"{ctx}\"\"\""""
    ...
    detalle = excerpt_from_segments(segments, ["exclusiv","plataforma","territorio","clientes"], 800)
    return {
...
\"\"\"{ctx}\"\"\""""
    ...
    detalle = excerpt_from_segments(segments, ["terminación","preaviso","vigencia","rescindir"], 800)
    return {
...
\"\"\"{ctx}\"\"\""""
    ...
    flat = "\n ".join(s["text"] for s in segments).lower()
    # safeguard con reglas
    if ("renovación automática" in flat or "renovacion automatica" in flat) and ("no " not in flat[:flat.find("renov")] if "renov" in flat else True):
        tiene_auto = "SI"
    if "sin renovaci" in flat or "no renovaci" in flat:
        tiene_auto = "NO"

    per = "NA"
    if any(x in flat for x in ["anual","cada 12 meses","cada año"]): per = "Anual"
    elif any(x in flat for x in ["semestral","cada 6 meses"]): per = "Semestral"
    elif "mensual" in flat: per = "Mensual"
    elif "renov" in flat: per = "Otro"

    pre_no_ren = extract_preaviso(flat) if tiene_auto == "SI" else "NA"
    detalle = excerpt_from_segments(segments, ["vigencia","renov","prórroga","extensi","duraci"], 800)

    return {
        "TieneRenovacionAutomatica": tiene_auto,
        "PeriodicidadRenovacion": per if tiene_auto == "SI" else "NA",
        "PreavisoNoRenovacion": pre_no_ren,
        "DetalleVigencia": detalle if "renov" in flat or "vigencia" in flat else "No hay nada significativo."
    }
